...
def get_creds():
    if os.environ.get("MIX_ACCOUNTS_JSON"):
        return json.loads(os.environ["MIX_ACCOUNTS_JSON"])
...
def id_to_string(frame):
    ...
    if len(cols) == 0:
        return
    res_frame = frame[cols].astype(str).map(add_A)
...
def string_to_id(frame):
    ...
    if len(cols) == 0:
        return
    #print('XXX', cols)
    frame[cols] = frame[cols].map(lambda x: int(str(x).replace('A', '')))
            ...
            for FROM_DATE, TO_DATE in zip(FROM_LIST, TO_LIST):
                ...
                if  not events_data_path.exists() and to_get_events:
                    ...
                    for assets_chunk in assets_chunks:
                        ...
                        if isinstance(date_report_events, pd.DataFrame) and not date_report_events.empty:
                            events_date_res.append(date_report_events)
                    if events_date_res:
                        events_date_df = pd.concat(events_date_res, ignore_index=True)
                ...
                if not trips_data_path.exists() and to_get_trips:
                    ...
                    for assets_chunk in assets_chunks:
                        ...
                        if isinstance(date_report_trips, pd.DataFrame) and not date_report_trips.empty:
                            trips_date_res.append(date_report_trips)
                    if trips_date_res:
                        trips_date_df = pd.concat(trips_date_res, ignore_index=True)
...
def read_daily_trips_file(path):
    ...
    if 'TripStart' in df.columns:
        df['TripStart'] = pd.to_datetime(df['TripStart'], errors='coerce')
    if 'TripEnd' in df.columns:
        df['TripEnd'] = pd.to_datetime(df['TripEnd'], errors='coerce')
    #print(path)
...
def make_daily_utilization(trips_df, FROM, TO):
    ...
    trips_df = trips_df.copy()
    if 'TripStart' in trips_df.columns:
        trips_df['TripStart'] = pd.to_datetime(trips_df['TripStart'], errors='coerce')
    if 'trip_day' in trips_df.columns:
        trips_df['trip_day'] = pd.to_datetime(trips_df['trip_day'], errors='coerce')
    ...
    trips2['TripStart'] = pd.to_datetime(trips2['TripStart'], errors='coerce')
    trips2['trip_day'] = pd.to_datetime(trips2['trip_day'], errors='coerce')
    trips2['weekday'] = trips2['TripStart'].dt.day_name()
    trips2.loc[trips2['weekday'].isna(), 'weekday'] = trips2.loc[trips2['weekday'].isna(), 'trip_day'].dt.day_name()
