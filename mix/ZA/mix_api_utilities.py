...
def detect_col_types(events_df):
    ...
    for event in events_df.EventName.unique():
        if 'night' in event.lower():
            distance_cols.append(event)
    for event in events_df.EventName.unique():
        if 'rpm' in event.lower():
            distance_cols.append(event)
...
def make_rag_score(trips_df, events_df, entity_column_name=None,
    ...
    if not entity_column_name:
        entity_column_name = ['AssetDescription']
...
possible_impact_comment = "This may lead to the shaft breaking, when the drive shaft breaks, the power from the engine can no longer reach the wheels, resulting in a loss of propulsion. Your vehicle may suddenly lose speed and become difficult to accelerate, making it challenging to maintain control."
...
def get_comment(event):
    ...
    if 'brak' in e:
        ...
        comment = possible_impact_comment
    return comment

def write_top_n(events_df, trips_df, n=3, bins=None, entity_column_name='DriverName'):
    if not bins:
        bins = [0, 20, 40, 100]
    res_out = []
    res_out2 = []
